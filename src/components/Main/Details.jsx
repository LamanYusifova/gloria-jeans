...
import { BasketContext } from '../Header/BasketContext'
import { WishlistContext } from '../Header/WishlistContext' // <- əlavə et

function Details() {
  const { addToBasket, basketPopUp, setBasketPopUp } = useContext(BasketContext)
  const { wishlistData, addToWishlist, removeFromWishlist } = useContext(WishlistContext) // <- əlavə et
  const { id } = useParams()
      ...
      <div className='grid grid-cols-2 max-md:grid-cols-1 gap-6 pt-20 px-4 md:px-20'>
        ...
        <div className='flex flex-col gap-6'>
          ...
          <div className='flex gap-4 items-center'>
            <button
              onClick={handleWishlistClick} // <- burda klik funksiyası
              className='py-3 px-5 bg-gray-300 rounded-md text-xl'
            >
