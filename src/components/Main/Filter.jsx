...
function Filter({ toggledFilter, filterPopUp, filterPopUp1, setFilterPopUp1, hideCategory = false }) {

  useEffect(() => {
    document.body.style.overflow = (filterPopUp || filterPopUp1) ? 'hidden' : 'auto';
    return () => {
      document.body.style.overflow = 'auto';
    };
  }, [filterPopUp, filterPopUp1]);

  return (
    <div
      className="bg-[#0009] fixed top-0 left-0 w-full h-full z-[1000] flex justify-center items-center"
      onClick={toggledFilter || (() => setFilterPopUp1?.(false))}
    >
      <div
      ...
      >
        ...
        <AccordionItem hideCategory={hideCategory} />
      </div>
    </div>
  );
}
